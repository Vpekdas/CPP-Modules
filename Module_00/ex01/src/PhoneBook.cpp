...
PhoneBook::PhoneBook() : _totalContacts(0), _nextIndex(0) {
  std::cout << BLUE << "👷 PhoneBook constructor called 👷" << RESET
            << std::endl;
}

PhoneBook::~PhoneBook() {
  std::cout << YELLOW << "🧨 PhoneBook Destructor called 🧨" << RESET
            << std::endl;
}
...
std::string validAddField(bool digitsOnly = false) {
  ...
  while (input.empty() || (digitsOnly && !onlyDigit(input))) {
    ...
    if (input.empty()) {
      std::cout << RED
                << "Error: Field cannot be empty. Please fill in the field."
                << RESET << std::endl;
    } else if (digitsOnly && !onlyDigit(input)) {
      std::cout << RED << "Error: Please enter only digits." << RESET
                << std::endl;
    }
    std::getline(std::cin, input);
  }
...
void addCommand(PhoneBook &phoneBook, std::size_t &index) {
  ...
  std::cout << GREEN << "Contact " NEON_GREEN << contact.getNickname() << GREEN
...
void PhoneBook::displaySpecificContact(std::size_t index) {
  if (index > this->_totalContacts) {
    std::cout << RED << "Error: Specified index is out of range." << NEON_RED
              << " Exiting SEARCH mode." << RESET << std::endl;
    return;
  } else if (this->_contacts[index].getFirstName().empty()) {
    std::cout << RED << "Error: No contact found at the specified index."
              << NEON_RED << " Exiting SEARCH mode." << RESET << std::endl;
    return;
  }
  std::cout << PURPLE << "All information for " << NEON_PURPLE
            << this->_contacts[index].getNickname() << ":" << RESET
            << std::endl;
  std::cout << NEON_CYAN << "First Name: " << RESET
            << _contacts[index].getFirstName() << std::endl;
  std::cout << NEON_CYAN << "Last Name: " << RESET
            << _contacts[index].getLastName() << std::endl;
  std::cout << NEON_CYAN << "Nickname: " << RESET
            << _contacts[index].getNickname() << std::endl;
  std::cout << NEON_CYAN << "Phone Number: " << RESET
            << _contacts[index].getPhoneNumber() << std::endl;
  std::cout << NEON_CYAN << "Darkest Secret: " << RESET
...
void PhoneBook::displayContacts() {
  std::string input;
  std::size_t index;
  std::cout << NEON_CYAN << "|" << "     INDEX" << "|" << "FIRST NAME" << "|"
            << " LAST NAME" << "|" << "  NICKNAME" << "|" << RESET << std::endl;
  for (std::size_t i = 0; i < _totalContacts; i++) {
    std::cout << "|" << "         " << this->_contacts[i].getIndex() << "|"
              ...
              << formatColumn(this->_contacts[i].getNickname()) << "|"
              << std::endl;
  }
  std::cout << NEON_CYAN << "Enter the contact index to see details." << RESET
            << std::endl;
...
void handleCommand(const std::string &command, PhoneBook &phoneBook,
  ...
  if (command == ADD_COMMAND) {
    ...
    std::cout << RED
              << "Error: The program accepts only these commands: " << NEON_RED
