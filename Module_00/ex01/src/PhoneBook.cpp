...
// Ensure that the field is always correct by repeatedly prompting the user
// until valid input is provided. This function handles both empty input and
// non-digit input (if digitsOnly is true) to maintain data integrity.
...
void addCommand(PhoneBook &phoneBook, std::size_t &index) {
    Contact contact;
    std::cout << NEON_CYAN << "🖊️ Enter first name 🖊️ : " << RESET;
    contact.setFirstName(validAddField());

    std::cout << NEON_CYAN << "🖊️ Enter last name 🖊️ : " << RESET;
    contact.setLastName(validAddField());

    std::cout << NEON_CYAN << "🆔 Enter nickname 🆔 : " << RESET;
    contact.setNickname(validAddField());

    std::cout << NEON_CYAN << "📞 Enter phone number 📞 : " << RESET;
    contact.setPhoneNumber(validAddField(true));

    std::cout << NEON_CYAN << "🤫 Enter darkest secret 🤫 : " << RESET;
    ...
    std::cout << GREEN << "Contact " NEON_GREEN << contact.getNickname() << GREEN << " added succesfully ✅ " << RESET
              << std::endl;
}
...
void PhoneBook::displaySpecificContact(std::size_t index) {
    if (index > this->_totalContacts) {
        std::cout << RED << "Error: Specified index is out of range." << NEON_RED << " Exiting SEARCH mode." << RESET
                  << std::endl;
        return;
    } else if (this->_contacts[index].getFirstName().empty()) {
        std::cout << RED << "Error: No contact found at the specified index." << NEON_RED << " Exiting SEARCH mode."
                  << RESET << std::endl;
        return;
    }
    std::cout << PURPLE << "All information for " << NEON_PURPLE << this->_contacts[index].getNickname() << ":" << RESET
              << std::endl;
    std::cout << NEON_CYAN << "First Name: " << RESET << _contacts[index].getFirstName() << std::endl;
    std::cout << NEON_CYAN << "Last Name: " << RESET << _contacts[index].getLastName() << std::endl;
    std::cout << NEON_CYAN << "Nickname: " << RESET << _contacts[index].getNickname() << std::endl;
    std::cout << NEON_CYAN << "Phone Number: " << RESET << _contacts[index].getPhoneNumber() << std::endl;
    std::cout << NEON_CYAN << "Darkest Secret: " << RESET << _contacts[index].getDarkestSecret() << std::endl;
}

void PhoneBook::displayContacts() {
    std::string input;
    std::size_t index;
    std::cout << NEON_CYAN << "|" << "     INDEX" << "|" << "FIRST NAME" << "|" << " LAST NAME" << "|" << "  NICKNAME"
              << "|" << RESET << std::endl;
    ...
    std::cout << NEON_CYAN << "Enter the contact index to see details." << RESET << std::endl;
...
void handleCommand(const std::string &command, PhoneBook &phoneBook, std::size_t &index) {
    if (command == ADD_COMMAND) {
        ...
        std::cout << NEON_CYAN << "👋 Bye bye 👋 " << RESET << std::endl;
        exit(0);
    } else {
        std::cout << RED << "Error: The program accepts only these commands: " << NEON_RED << "ADD, SEARCH, EXIT."
                  << RESET << std::endl;
    }
}