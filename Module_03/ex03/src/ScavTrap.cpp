...
void ScavTrap::attack(const std::string &target) {
    ...
    std::cout << "🗡️ " << NEON_BLUE << "[ScavTrap " << getName() << "]" << BLUE << " has attacked " << NEON_BLUE
              << target << BLUE << " with " << getAttackDamage() << " attack damage. " << getName() << " has "
              << getEnergyPoints() << " energy points left." << RESET << std::endl;
}

void ScavTrap::guardGate() {
    setEnergyPoints(getEnergyPoints() - 1);
    std::cout << "🛡️ " << NEON_BLUE << getName() << BLUE << " is protecting themselves. " << getName() << " has "
              << getEnergyPoints() << " energy points left." << RESET << std::endl;
}
