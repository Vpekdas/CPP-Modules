...
const ErrorTypeMapping typeMappings[] = {
    {BUREAUCRAT, "{BUREAUCRAT}"}, {FORM, "{FORM}"}, {INCREMENT, "{INCREMENT}"}, {DECREMENT, "{DECREMENT}"}};
...
void Bureaucrat::incrementGrade() {
    if (_grade - 1 <= 0) {
        throw GradeTooHighException(_name, _grade, INCREMENT);
    }
    _grade--;
}
void Bureaucrat::decrementGrade() {
    if (_grade + 1 > 150) {
        throw GradeTooLowException(_name, _grade, DECREMENT);
    }
    _grade++;
}
