  ...
  if (inputFile.is_open() && outputFile.is_open()) {
    while (std::getline(inputFile, line)) {
      ...
      while (indexOccurence != -1) {
        std::string nextLine = line.substr(indexOccurence + occurenceLength,
                                           initialLength - indexOccurence);
        std::string previousLine = line.substr(0, indexOccurence) + replaceBy;
        line = previousLine + nextLine;
        indexOccurence =
            line.find(occurence, indexOccurence + replaceBy.length());
      }
      outputFile << line;
      if (!inputFile.eof()) {
        outputFile << std::endl;
    ...
    std::cout << NEON_GREEN << "File copied successfully." << RESET
              << std::endl;
  } else {
